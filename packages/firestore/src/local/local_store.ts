...
export class LocalStore {
    ...
    return this.persistence.runTransaction('Reject batch', true, txn => {
      ...
      return this.mutationQueue
        ...
        .next((promisedToReject: MutationBatch | null) => {
          ...
          affectedKeys = promisedToReject.keys();
  ...
  private applyWriteToRemoteDocuments(
    ...
    // todo: change removeMutationBatches to accept a single batch and return no value
    return promiseChain.next(() =>
      this.removeMutationBatch(txn, batchResult.batch).next(() => {})
    );
  }
